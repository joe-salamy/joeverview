...
echo "$KEYS" | grep -q 'bind-key -T prefix o .*display-popup -B.*tmux-window-picker' \
  && echo "OK   prefix o (borderless grid)" || { echo "FAIL prefix o"; fail=1; }
echo "$KEYS" | grep -q 'bind-key -T prefix O .*tmux-pane-picker' \
  && echo "OK   prefix O" || { echo "FAIL prefix O"; fail=1; }
echo "$KEYS" | grep -q 'bind-key -T prefix / .*tmux-content-search' \
