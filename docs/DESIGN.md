...
`prefix o` shows one session at a time (`list-sessions`, then per-session `list-windows -t $sid`). The switcher (`← name →`) is stamped into the existing top border next to ` Windows `, so grid geometry and the subtractive frame are unchanged; bounds just narrow from global `n` to the visible session's `[s0, send)`. Up from the top grid row focuses the bar (always, even single-session), Left/Right pages (reset to the session's first page, full `draw` — the only bar painter, so no partial-repaint desync), Down returns, Enter attaches (`switch-client`) or jumps (`switch-client` + `select-window -t @id` when the window lives elsewhere). Titles carry `⧉ N` from `#{window_panes}` when N>1; the body stays the active-pane capture, so exact-pane work stays on `prefix O`.
